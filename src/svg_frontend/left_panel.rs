    ...
    for (hash, timeline) in timelines {
        ...
        for (line_number, event) in timeline.history.iter() {
            ...
            match event {
                ...
                Event::MutableLend { to : to_ro2 } => {
                    if let Some(ro2) = to_ro2 {
                        let data = ArrowData {
                            x1: ro1_x_pos,
                            y1: ro1_y_pos,
                            x2: resource_owners_layout[&ro2.hash].x_val-10,
                            y2: get_y_axis_pos(line_number)
                        };
                        output.push_str(&registry.render("arrow_template", &data).unwrap());
                    }
                },
                Event::MutableReturn { to : to_ro2 } => {
                    if let Some(ro2) = to_ro2 {
                        let data = ArrowData {
                            x1: ro1_x_pos,
                            y1: ro1_y_pos,
...
fn get_vertical_line_class(state : &State) -> String {
    ...
    match state {
        ...
        // State::RevokedPrivilege{ to: _, borrow_to: _ } => "dotted".to_string(),
        _ => "".to_string(),
    }
}
